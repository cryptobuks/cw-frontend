      ...
      <div class="flex justify-between w-full">
        <div class="flex items-center">
          ...
          <h2 class="font-extrabold text-md">
            {{ $t("report.trend_contacts") }}
          </h2>
          <base-icon
            v-tippy="{
              content: $t('report.tooltip'),
              placement: 'bottom',
            }"
  ...
  computed: {
    ...
    options () {
      return {
        ...
        series: [
          ...
          {
            data: this.getGraphData.map(item => item.count),
            type: 'bar',
            xAxisIndex: 1,
            name: this.getCurrentType.name,
            barWidth: '50%',
            itemStyle: {
    ...
    getGraphData () {
      ...
      for (const key in stats) {
        const filteredStat = Object.values(stats[key].data).map(
          item => item[this.getCurrentType.type]
        )
    ...
    graphMax () {
      return (
        _.ceil(
          _.max([
            _.maxBy(this.getGraphData, 'count')?.count,
            _.maxBy(this.getGraphData, 'budget')?.budget,
            _.maxBy(this.getGraphPrevious, 'count')?.count,
            _.maxBy(this.getGraphPrevious, 'budget')?.budget
          ]),
          -1
        ) + 10
      )
