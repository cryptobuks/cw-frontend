      ...
      <div class="grid grid-cols-3 w-full">
        <div class="flex items-center">
          ...
          <h2 class="font-extrabold text-md">
            {{ $t("report.active_sport") }}
          </h2>
          <base-icon
            v-tippy="{
              content: $t('report.sport_tooltip'),
              placement: 'bottom',
            }"
...
export default {
  ...
  computed: {
    ...
    getGraphData () {
      ...
      const reformed = {}
      const max = stats?.reduce((acc, { data }, index, arr) => {
      ...
      max?.forEach((element, index) => {
        Object.values(element).forEach((sport) => {
          if (reformed[sport.sportInterestId]) {
            reformed[sport.sportInterestId][index] = sport.count
          } else {
            reformed[sport.sportInterestId] = Array(max).fill('_')
            reformed[sport.sportInterestId][index] = sport.count
          }
        })
      })
  ...
  async mounted () {
    this.months = await this.$repos.statRepo.getSportContactsMonth()
    await this.getStats()
  ...
  methods: {
    ...
    async getStats (
      ...
      this.stats = await this.$repos.statRepo.getSportContactsStats({
        periods: payload
      })
    },
    getValues (value) {
