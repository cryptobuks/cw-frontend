  ...
  <div class="fixed w-full h-full overflow-auto flex flex-col text-center text-white">
    ...
    <main class="px-5 sm:px-6 overflow-hidden flex-shrink-0 flex-grow relative max-w-lg w-full mx-auto">
      <template v-if="!isSubmitted">
        ...
        >
          <template #message="{ inputAttrs }">
            <base-textarea
              v-model="draft.message"
              v-bind="inputAttrs"
              :placeholder="$t('ask_info.message.placeholder')"
              maxlength="300"
            />
          </template>
      ...
      <div v-else class="text-center">
        ...
        <p>
          {{ isDuplicated ? $t('book_shift.booking_existed_for') : $t('ask_info.thank_you') }}

          <span v-if="isDuplicated" class="text-4xl">{{ [draft.firstname, draft.lastname].filter(Boolean).join(' ') }}</span>
        </p>

        <p v-if="bookingNumber" class="mt-12">
          {{ $t('ask_info.booking_number', { bookingNumber }) }}
        </p>
      </div>
    </main>
    ...
    <footer class="px-5 sm:px-6 mt-12 flex-shrink-0 max-w-lg w-full mx-auto pb-10 flex">
      ...
      <template v-else>
        <base-button xl inline class="mr-5 flex-shrink-0" @click="back">
          <base-icon name="dismiss" class="text-3xl" />
        </base-button>

        <base-button xl class="uppercase" @click="goToBookShift">
          {{ $t('ask_info.wait') }}
        </base-button>
      </template>
    </footer>
...
export default {
  data () {
    return {
      fields: [
        ...
        ...f,
        label: this.$t('ask_info.' + f.name + '.label'),
        tooltip: this.$t('ask_info.' + f.name + '.tooltip')
      })),
  ...
  methods: {
    ...
    async submit () {
      ...
      const res = await this.$repos.gymDeviceRepo.askInfo({
        userId: profile._id,
        question: this.draft.message
      })
    ...
    back () {
      ...
      this.$router.push({ path: '/device', query: this.$route.query })
    },

    goToBookShift () {
      const displayName = [this.draft.firstname, this.draft.lastname].filter(Boolean).join(' ')
      this.$router.push({
        path: '/device/book-your-shift',
        query: {
          done: true,
          displayName,
