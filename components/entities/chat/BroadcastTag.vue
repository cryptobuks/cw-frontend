<template>
  <div class="tag-wrapper">
    <div v-if="$slots.icon || type" class="flex-shrink-0 w-3 h-3 lg:w-4 lg:h-4">
      <slot v-if="$slots.icon" name="icon" />
      <svg v-else-if="type" xmlns="http://www.w3.org/2000/svg" class="fill-current stroke-current w-full h-full">
        <use :href="`#${type}-svg-icon-${_uid}`" />
      </svg>
    </div>
    <div class="truncate select-none ml-1">
      <slot />
    </div>
    ...
    <svg xmlns="http://www.w3.org/2000/svg" class="hidden">
      ...
      <symbol :id="`course-svg-icon-${_uid}`" viewBox="0 0 21 22">
        <g transform="translate(-36.506 .09)" fill="none" stroke-width=".95">
          <path d="M38.885 4.449a.25.25 0 000 .352l1.404 1.424h.002a.25.25 0 00.353 0l1.65-1.65a.25.25 0 00.001-.352L40.89 2.8l-.001-.001a.25.25 0 00-.354 0z" />
          <rect
            transform="rotate(-45)"
            x="19.773"
            y="33.158"
            ...
            rx="1"
          />
          <path d="M53.463 19l-.002.002a.25.25 0 01-.353-.004l-1.397-1.432a.25.25 0 01.002-.351l1.65-1.65.002-.002a.25.25 0 01.354.004L55.115 17a.25.25 0 01-.002.351z" />
          <rect
...
export default {
  props: {
    // 'profile', 'course', 'interest' or 'role'
...
.tag-wrapper {
  ...
  @apply inline-flex items-center text-xs text-white px-2 py-1 rounded-lg;
}
</style>
